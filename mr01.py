...
class mario ():
    ...
    def update_sub(self, brick):
        ...
        if self.jf == 0:
            ...
            #それ以外
                ...
                if (brick.rry <= self.my+self.baseh <= brick.rry+15) and (brick.rrx-self.basew < self.mx < brick.rrx+brick.rrw):
                    #下に台がある場合
                    # 右端にいる　e5.png
                    if brick.rrx +brick.rrw -20 < self.mx < brick.rrx+brick.rrw:
                        self.mx +=20
                    # 左端にいる　e6.png
                    elif brick.rrx < self.mx+self.basew < brick.rrx+20:
                        self.mx -=20
                    else:    
                        self.mx += self.vx
        ...
        #マリオがジャンプ
            ...
            if self.vx == 0 and brick.rrx-self.basew < self.mx < brick.rrx+brick.rrw and  brick.rry < self.my+self.baseh < brick.rry+50:
                self.vy = 0
                self.jf = 0
                self.my = brick.rry-self.baseh
            #マリオが右からジャンプして着地、上の壁検出
            elif self.vx < 0 and brick.rrx-self.basew < self.mx < brick.rrx+brick.rrw and  brick.rry < self.my+self.baseh < brick.rry+50:
                self.vy = 0
                self.jf = 0
                self.my = brick.rry-self.baseh
            #マリオが左からジャンプして着地、上の壁検出
            elif self.vx > 0 and brick.rrx < self.mx+self.basew < brick.rrx+brick.rrw+self.basew and  brick.rry < self.my+self.baseh < brick.rry+50:
                self.vy = 0
                self.jf = 0
                self.my = brick.rry-self.baseh
            #それ以外
    ...
    def draw_text(self, screen, font, brick):
        #マリオの位置　　　
        t = "mx=" + str(self.mx) + "  my=" + str(self.my)
